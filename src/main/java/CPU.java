...
public class CPU
{
    ...
    public void execute(int opcode)
    {
        ...
        { //want only the first byte of the opcode initially
        case 0x0000:
...
//                PC += 2;
        ...
        case 0x8000:
            ...
            case 0x0006: //case 0x8xy6 //SHR
                ...
                System.out.println("SHR V" + Integer.toHexString(x) + " V" + Integer.toHexString(y) + "(v" + Integer
                    .toHexString(y) + " is ignored)");
            ...
            case 0x0007: //case 0x8xy7 //SUBN
                ...
                if (V[x] <= V[y])
                { //TODO: >= based on Jchip, used to be just >
                    V[0xF] = 1;
                }
                else
                {
        ...
        case 0xD000: //case 0xDxyn //DRW Vx, Vy, nibble
            ...
            System.out.println("DRW V" + Integer.toHexString(x) + " V" + Integer.toHexString(y) + " " + Integer
                .toHexString(n));

            //read n bytes from memory, starting at I
            for (int i = 0; i < n; i++)
            { //loop for each byte (row) in the sprite
                int data = memSystem.getValue(I + i); //retrives the byte for the given line of pixels
                for (int j = 0; j < 8; j++)
                { //this loop is for each bit in the byte
                    //get current pixel value
                    if (V[x] + j < 64 && V[y] + i < 32)
                    { //TODO: Cowgod specifies we should loop round, but no implementation currently does this
                        int curPix = display.getValue((V[x] + j), (V[y] + i));
                        //get the pixel value from the byte in the data
                        int newPix = data & (0x80 >> j);

                        if (newPix > 0)
                        { //TODO: These two ifs can go?
                            newPix = 1;
                        }
                        if (curPix > 0)
                        {
